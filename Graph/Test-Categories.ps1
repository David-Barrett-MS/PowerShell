...
<#
.SYNOPSIS
Script to test creating and listing mailbox categories via Microsoft Graph API.

.DESCRIPTION
This script connects to a mailbox using Microsoft Graph API and checks for the existence of specific categories. If the categories do not exist, it creates them. Finally, it lists all categories in the mailbox.
Specify categories to be created using the -RequiredCategories parameter.  Default categories are:
    MyCategory1 - preset18
    MyCategory2 - preset19
    MyCategory3 - preset20
    MyCategory4 - preset21

.EXAMPLE
.\Test-Categories.ps1 -AppId $clientId -AppSecretKey $secretKey -TenantId $tenantId -User $Mailbox
...
<# Graph.ps1 %PARAMS_END% #>
    ...
    [Parameter(Mandatory=$false,HelpMessage="Categories to be created if they don't already exist.")]
    [ValidateNotNullOrEmpty()]
    $RequiredCategories = @(
        @{
            "displayName" = "MyCategory1";
            "color" = "preset18"
        },
        @{
            "displayName" = "MyCategory2";
            "color" = "preset19"
        },
        @{
            "displayName" = "MyCategory3";
            "color" = "preset20"
        },
        @{
            "displayName" = "MyCategory4";
            "color" = "preset21"
        }
    )
)
...
# List current categories in the mailbox
$categories = GET "$($script:graphBaseUrl)outlook/masterCategories"
if ($null -eq $categories -or $categories.Count -eq 0)
{
    Log "Unable to retrieve master category list" Red
    exit
}
$global:cats = (ConvertFrom-Json $categories).value

# Check if each custom category exists, and create if not
$addedCategory = $false
foreach ($cat in $RequiredCategories)
{
    $existingCat = $global:cats | Where-Object { $_.displayName -eq $cat.displayName }
    if ($null -eq $existingCat)
    {
        Log "Creating category: $($cat.displayName)"
        $body = $cat | ConvertTo-Json
        $result = POST "$($script:graphBaseUrl)outlook/masterCategories" $body
        if ($null -ne $result)
        {
            Log "Successfully created category: $($cat.displayName)" Green
            $addedCategory = $true
        }
    }
}
...
if ($addedCategory)
{
    # List updated categories in the mailbox
    $categories = GET "$($script:graphBaseUrl)outlook/masterCategories"
    $global:cats = (ConvertFrom-Json $categories).value
}
